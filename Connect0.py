...
from PyQt5.QtWidgets import QApplication, QListWidgetItem, QWidget, QVBoxLayout, QHBoxLayout
from PyQt5.QtCore import Qt, QObject, pyqtSlot, pyqtSignal, QStringListModel, QSize
...
class Connect(Ui_XuanNiaoTR):
    addBubbleSignal = pyqtSignal(dict)
    ...
    def __init__(self):
        ...
        self.connecting()
        # self.ClientThread.signal.connect(self.Refresh)
    ...
    def add_content(self, bubble, send, file, file_name=None, text=None):
        ...
        if bubble:
            ...
            # Bubble.addList(self, Msg)
            # 让AddBubbleSignal向scrollAreaWidgetContents发射增加frame的信号
            Connect.addBubbleSignal.emit(Msg)
        else:
            # 发送
            print("是文本模式")
            if send:
    ...
    def connecting(self):
        thread_c = threading.Thread(target=self.connect)
        thread_c.start()
        return

    def sending(self):
        thread_s = threading.Thread(target=self.send_data)
        thread_s.addBubbleSignal.connect(Bubble.addList)
        thread_s.start()
        print("触发发送1")
        return
    ...
    def receiving(self):
        sleep_time = 5
        thread_r = threading.Thread(target=self.receive_data(), args=(sleep_time,))
